...
import { AlertTriangle, UsersRound } from "lucide-react-native";
...
export function PersonaGroupScreen() {
  const { updateUser, user } = useAuth();
  const { error, groups, loading } = usePersonaData(user, updateUser);
  ...
  const showFutureAction = () => {
    Alert.alert(
      "Agregar integrante",
      "La estructura movil esta lista; falta convertir este boton en un formulario para consumir el endpoint."
    );
  };
  ...
  return (
    <GradientScreen>
      <View style={styles.header}>
        <Pill icon={UsersRound} tone="green">
          Mi grupo
        </Pill>
        <Text style={styles.title}>Grupos familiares y contactos</Text>
        <Text muted style={styles.subtitle}>
          Revisa quienes estan compartiendo, pausados o sin conexion.
        </Text>
      </View>
      ...
      {loading ? <LoadingView message="Cargando grupo..." /> : null}
